...
/* Keep every macro at or above a small positive floor so the pair scorer stays stable
   when a slot has already been overshot. */
export function clampPositive(m, floor = 1) {
  return { kcal: Math.max(m.kcal, floor), protein: Math.max(m.protein, floor),
           fat: Math.max(m.fat, floor), carbs: Math.max(m.carbs, floor) };
}
